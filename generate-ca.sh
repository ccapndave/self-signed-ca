...
mkdir $TARGET_DIR
openssl genrsa -passout pass:$PASSWORD -des3 -out ${TARGET_DIR}/${ORGANISATION}CA.key 2048
openssl req -passin pass:$PASSWORD -x509 -new -nodes -key ${TARGET_DIR}/${ORGANISATION}CA.key -sha256 -days 36500 -subj "/CN=${ORGANISATION} DEV CA/O=${ORGANISATION}/C=CH" -out ${TARGET_DIR}/${ORGANISATION}CA.pem
...
echo "openssl x509 -req -passin pass:$PASSWORD -in \$CERTIFICATE_NAME.csr -CA ${ORGANISATION}CA.pem -CAkey ${ORGANISATION}CA.key -CAcreateserial -out \$CERTIFICATE_NAME.crt -days 825 -sha256 -extfile <(printf \"subjectAltName=\$SERVERS\")" >> $SCRIPT
